...
wx.MessageBox(' -_-  Add Extension and Then Click On OK BUTTON -_- ', 'Special Google Translation', wx.OK | wx.ICON_ERROR)
time.sleep(5)
...
def connection():
    ...
    while a3 == 0:
        try:
            connection = pymysql.connect(host=str(Server_id),
                                         user='ams',
                                         password='amsbind',
...
def tarnslation():
    try:
        ...
        trasns = connection()
        cur = trasns.cursor()
        cur.execute(str(query))  # 0 = English, 1 = Non-English
        rows = cur.fetchall()
                ...
                if not re.match("^[\W A-Za-z0-9_@?./#&+-]*$", notice_no):
                    ...
                    if is_available == 1:
                        wx.MessageBox(
                            ...
                            'Special Google Translation ', wx.OK | wx.ICON_WARNING)
                    else:
                        time.sleep(5)
        ...
        wx.MessageBox('All Process Done','Special Google Translation ', wx.OK | wx.ICON_INFORMATION)
        time.sleep(2)
        browser.close()
        sys.exit()
        time.sleep(2)
